...
export const addToast = createAsyncThunk(ToastActions.ADD_TOAST, async ({ level, title, description }: CreateToast, { dispatch }) => {
  const id = nanoid();
  ...
  return ({
    id,
    level,
    title: typeof title === 'function' ? title(i18n.t) : title ?? getDefaultLabel(level, i18n.t),
    description: typeof description === 'function' ? description(i18n.t) : description,
  });
});
