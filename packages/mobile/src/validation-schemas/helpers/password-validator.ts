...
const passwordValidator =
  (t: TFunction) => (value: string, helpers: Joi.CustomHelpers) => {
    ...
    if (value.match(/\s/)) {
      return helpers.message({ custom: t('errors.SPACES') });
    }

    if (value.length < MIN_PASSWORD_LENGTH) {
      return helpers.message({
        custom: t('errors.MIN_PASSWORD_LENGTH'),
      });
    }

    if (value.length > MAX_PASSWORD_LENGTH) {
      return helpers.message({
        custom: t('errors.MAX_PASSWORD_LENGTH'),
      });
    }

    if (!hasUppercase || !hasLowercases || !hasNumber) {
      return helpers.message({
        custom: t('errors.UP_LOW_NUM'),
      });
    }

    if (!value.match(PASSWORD_REGEX)) {
      return helpers.message({ custom: t('errors.INVALID_PASSWORD') });
    }
